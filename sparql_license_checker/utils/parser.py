    ...
    try:
        ...
        for i, t in enumerate(terms):
            ...
            if t == ',':
                # inference
                try:
                    if 'OptionInference' in res[-1].name :
                        additional_index = 1
                except:
                    pass

                res.append(res[i - 3 - additional_index])
                res.append(res[i - 2 - additional_index])
            elif t == ';':
                # inference
                try:
                    if 'OptionInference' in res[-1].name :
...
Define = Comp('Define', Keyword('DEFINE') + Param('param1', PrefixedName) + Param('param2', (PrefixedName | IRIREF | String)) ) 

# inference
# [4] Prologue ::= ( BaseDecl | PrefixDecl )*
Prologue = ZeroOrMore(Define) + Group(ZeroOrMore(BaseDecl | PrefixDecl))
...
#################################################################
# simple changes to manage virtuoso's option inference clause :
#################################################################
...
TripleOption = Keyword('INFERENCE') + (Param('param', iri) | Param('param', String))

#[Virt]  TripleOptions    ::=  'OPTION' '(' TripleOption ( ',' TripleOption )? ')'
TripleOptions = Keyword('OPTION') + '(' + TripleOption + Optional(',' + TripleOption) + ')'

# [86] ObjectListPath ::= ObjectPath ( ',' ObjectPath )*
ObjectListPath = ObjectPath + Optional(Comp('OptionInference',TripleOptions)) + ZeroOrMore(',' + ObjectPath) + Optional(Comp('OptionInference',TripleOptions))
...
# [83] PropertyListPathNotEmpty ::= ( VerbPath | VerbSimple ) ObjectListPath ( ';' ( ( VerbPath | VerbSimple ) ObjectList )? )*
PropertyListPathNotEmpty = (VerbPath | VerbSimple) + ObjectListPath + ZeroOrMore(
    ';' + Optional((VerbPath | VerbSimple) + ObjectList) + Optional(Comp('OptionInference',TripleOptions)))
#################################################################
#################################################################
#################################################################
...
BrackettedExpression = Comp('BrackettedExpression', Suppress('(') + Param('expr', Expression) + Suppress(')') )
