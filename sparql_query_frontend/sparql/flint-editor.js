...
function FlintEditor(container, imagesPath, config) {
	...
	function FlintResults(editor) {
		...
		this.setResults = function(text, endpoint) {
			results = text;
			if ($.isXMLDoc(results)) {
										...
										resultItem.find(
												...
												function() {
													$(this).find("*").each(
															function() {
																if (this.tagName === "URI") {
																	devResults += "<td><a href='http://log.disit.org/service/?uri=" + 
                                    encodeURI($(this).text()) + "&sparql=" +
                                    encodeURI(endpoint)+"' target='_blank'>" + $(this).text()
																			+ "</a></td>";
																} else {
	...
	function FlintEndpointEntry(config, editor) {
		...
		try {
			...
			this.display = function(container) {
				var endpoint = 'http://www.eclap.eu/sparql';
				$('#' + container)
						.append(
	...
	function FlintCoolbar(config, editor) {
		...
		try {
			...
			coolbarItems.push(editor.licenseChecker.getLicenseCheckButton(editor));
			coolbarItems.push(editor.licenseChecker.getCategorySelectionField(editor));
			this.getItems = function() {
				return coolbarItems;
			};
		} catch (e) {
	...
	function FlintSidebar(editor, config) {
		...
		function showTab(tabName, id) {
			...
			if (tabName == "Properties") {
				...
				displayPrefixes();
			} else if (tabName === "SPARQL") {
				displaySparql();
			} else {
				$('#flint-sidebar-samples').attr("class", "flint-sidebar-selected");
				displaySamples();
			}
		}
		...
		this.updateProperties = function(datasetItem) {
			try {
				...
				if (datasetItem.properties == null) {
					...
					this.showActiveTab();
					var paramsData = {};
					paramsData[config.defaultEndpointParameters.queryParameters.query] = "SELECT DISTINCT ?p WHERE {?p a <http://www.w3.org/1999/02/22-rdf-syntax-ns#Property>} ORDER BY ?p"; //"SELECT DISTINCT ?p WHERE {?s ?p ?o} ORDER BY ?p LIMIT 1000";
          paramsData['service-uri'] = datasetItem.uri;
          paramsData['timeout'] = 30;
          paramsData['cache'] = 'properties';
					$
							.ajax({
								url : 'sparql-proxy.php',
								data : paramsData,
								type : 'get',
								headers : {
		...
		this.updateClasses = function(datasetItem) {
			try {
				...
				if (datasetItem.classes == null) {
					...
					this.showActiveTab();
					var paramsData = {};
					paramsData[config.defaultEndpointParameters.queryParameters.query] = "SELECT DISTINCT ?o WHERE {?o a <http://www.w3.org/2002/07/owl#Class> FILTER(isURI(?o))} ORDER BY ?o"; //"SELECT DISTINCT ?o WHERE {?s a ?o} ORDER BY ?o LIMIT 1000";
          paramsData['service-uri'] = datasetItem.uri;
          paramsData['timeout'] = 30;
          paramsData['cache'] = 'classes';
					$
							.ajax({
								url : 'sparql-proxy.php',
								data : paramsData,
	...
	function FlintCodeEditor(flint, editorMode) {
		...
		function cmUpdate() {
			...
			if (state.complete) {
				...
				//flint.getCoolbar().getItems()[4].enable();
				...
				//flint.getCoolbar().getItems()[4].disable();
	...
	function Flint(container, imagesPath, config) {
        ...
        flint.licenseChecker = new LicenseChecker(flint, editorId, config);
		...
		try {
          ...
          /*paramsData['timeout'] = 300;*/
					...
					$.ajax({
						...
						success : function(data) {
							resultsArea.setResults(data, datasetItems.getEndpoint());
						}
					});
				} catch (e) {
			...
			this.sendEndpointQuery = function() {
				try {
					...
					paramsData[paramsDataItem] = cm.getCodeMirror().getValue();
          paramsData['service-uri'] = endpointItem.getEndpoint();
          /*paramsData['timeout'] = 300;*/
					...
					$.ajax({
						...
						success : function(data) {
							resultsArea.setResults(data, endpointItem.getEndpoint());
						}
					});
				} catch (e) {
            ...
            //license-checker 
            this.licenseChecker.setButtonAction(resultsArea, errorBox, config, cm, datasetItems, datasetMimeTypeItem, flint);
