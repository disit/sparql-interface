...
function spqllog($e) {
  $f=fopen('proxy.log','at');
  fwrite($f, date('r').'|'.$_SERVER['REMOTE_ADDR'].'|'.$_REQUEST['service-uri']."|$e\n");
  fclose($f);
}
...
function processHeader($responseHeader,$ignoreResponseHeaders) {
...
if(isset($_REQUEST['cache'])) {
  $cache='cache/'.urlencode($serviceUrl).'-'.$_REQUEST['cache'];
  if(file_exists($cache.'.hdr') && file_exists($cache.'.bdy')) {
    processHeader(file_get_contents($cache.'.hdr'),$ignoreResponseHeaders);
    echo file_get_contents($cache.'.bdy');
    exit;
  }
}
...
if($ch === FALSE) {
    ...
    spqllog('Failed to initialize curl');
    die;
}
...
processHeader($responseHeader,$ignoreResponseHeaders);
...
if(isset($cache) && $statusCode==200) {
  $f=fopen($cache.'.hdr','wb');
  fwrite($f,$responseHeader);
  fclose($f);
  $f=fopen($cache.'.bdy','wb');
  fwrite($f,$body);
  fclose($f);
}
